...
namespace BusinessLogic.Helpers
{
    public class PaginationTagHelper : TagHelper
    {
        ...
        private void ClosingHtml(StringBuilder content)
        {
            ...
            {
                content.Append($"<li class='page-item'><a class='page-link' href='{PageTarget}?pageNumber={PageCount}'>{PageLast}</a></li>");
            }
        ...
        private void PageNumberUpperPageRange(StringBuilder content)
        {
            for (int currentPage = PageCount - (2 * PageRange); currentPage < PageCount + 1; currentPage++)
            {
                ...
                content.Append($"<li class='page-item {active}'><a class='page-link'href='{PageTarget}?pageNumber={currentPage}'>{currentPage}</a></li>");
            }
        }
